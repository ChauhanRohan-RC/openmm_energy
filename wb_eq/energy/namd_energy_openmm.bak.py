...
TODO: severe memory leaks and crashes due to OOM
Created: Sep 16, 10 PM
...
# ------------------------------------------------------------------------
raw_requested = set([e.lower().strip().lstrip('-') for e in OUT_ENERGIES])
if "all" in raw_requested:
    raw_requested.update(["vdw", "elec", "nonb", "pote", "total"])
    if is_self_interaction: raw_requested.update(["bond", "angl", "dihe", "impr", "conf"])
if "total" in raw_requested or "pote" in raw_requested:
    raw_requested.update(["vdw", "elec", "nonb"])
    if is_self_interaction: raw_requested.update(["bond", "angl", "dihe", "impr", "conf"])
...
if IS_DYNAMIC:
    ...
    log_info("Static Selections detected. Engaging Static Fast-Path Native Topology.")
...
for custom_f in [vdw_force, elec_force]:
    custom_f.setNonbondedMethod(
        mm.CustomNonbondedForce.CutoffPeriodic if PERIODIC else mm.CustomNonbondedForce.CutoffNonPeriodic)
...
if is_self_interaction and any(e in final_components for e in ["bond", "angl", "dihe", "impr", "cmap"]):
    ...
    for f in base_system.getForces():
        ...
        if fname == "HarmonicBondForce":
            ...
            if not is_ub and "bond" in final_components:
            ...
            elif is_ub and "angl" in final_components:
                new_f = mm.HarmonicBondForce()
        ...
        elif fname == "HarmonicAngleForce" and "angl" in final_components:
        ...
        elif fname == "PeriodicTorsionForce" and "dihe" in final_components:
        ...
        elif fname == "CustomTorsionForce" and "impr" in final_components:
        ...
        elif fname == "CMAPTorsionForce" and "dihe" in final_components:
...
active_fetches = [(1 << group_map[c], comp_idx[c]) for c in final_components]
...
while not SHUTDOWN_REQUESTED:
    ...
    if PERIODIC: context.setPeriodicBoxVectors(box_nm[0], box_nm[1], box_nm[2])
    ...
    if PERIODIC and PME_ENABLED and is_elec_requested:
        if is_self_interaction:
            ...
            if OUT_FORCE:
                f_pme_cross_raw = st_AB.getForces(asNumpy=True).value_in_unit(to_kcal_A) - st_A.getForces(asNumpy=True).value_in_unit(to_kcal_A)
...
output_file = f"{OUT_FILE_PREFIX}.csv"
with open(output_file, 'w') as f_out:
    ...
    if "bond" in raw_requested: headers.append("BOND")
    if "angl" in raw_requested: headers.append("ANGLE")
    if "dihe" in raw_requested: headers.append("DHIED")
    if "impr" in raw_requested: headers.append("IMPRP")
    if "conf" in raw_requested: headers.append("CONF")
    if "elec" in raw_requested: headers.append("ELECT")
    if "vdw" in raw_requested: headers.append("VDW")
    if "nonb" in raw_requested: headers.append("NONBOND")
    if "pote" in raw_requested: headers.append("POTENTIAL")
    if "total" in raw_requested: headers.append("TOTAL")
    ...
    for row in data_buffer:
        ...
        if "bond" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(e_bond))
        if "angl" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(e_angl))
        if "dihe" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(e_dihe))
        if "impr" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(e_impr))
        if "conf" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(conf_eng))
        if "elec" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(e_elec))
        if "vdw" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(e_vdw))
        if "nonb" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(nonb_eng))
        if "pote" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(pote_eng))
        if "total" in raw_requested: out_row.append(OUT_ENERGY_FORMAT.format(pote_eng))
